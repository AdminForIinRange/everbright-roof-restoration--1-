...
export function AssessmentForm() {
  ...
  const panelCondition = "Free solar cleaning quote request"
  const { error, formAction, handleSubmit, pending, sourcePath } = useSolarCleaningLeadAction({
    formContext: "solar-cleaning-assessment",
    panelCondition,
    ...
    <section id="assessment" className="border-t border-border bg-secondary px-4 py-12 md:px-10 md:py-20 lg:px-20">
      <div className="mx-auto max-w-lg text-center md:max-w-3xl">
        ...
        <h2 className="mb-3 font-heading text-3xl tracking-wide text-foreground md:mb-4 md:text-5xl">
          Start your free solar cleaning quote
        </h2>
        ...
        {submitted ? (
          ...
          >
            ...
            <input type="hidden" name="message" value={panelCondition} />
            <input type="hidden" name="formSource" value="solar-cleaning-assessment" />
            ...
            {FIELDS.map((field) => (
              <div key={field.key} className="mb-4 text-left">
                <label
                  htmlFor={`solar-assessment-${field.key}`}
                  className="mb-1 block text-xs font-semibold uppercase tracking-wide text-muted-foreground"
                >
                  {field.label}
                </label>
                <input
                  id={`solar-assessment-${field.key}`}
                  name={field.key}
                  type={field.type}
                  required
