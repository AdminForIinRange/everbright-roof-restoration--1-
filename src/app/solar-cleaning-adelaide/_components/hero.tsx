...
import { useSolarCleaningLeadAction } from "./use-solar-cleaning-lead-action";

const OPTIONS = [
  "Light dust",
  "Bird droppings / grime",
  "Heavy build-up",
  "Maintenance clean",
];
...
export function Hero() {
  ...
  const panelCondition = q1Selected ?? "Maintenance clean";
  const { error, formAction, handleSubmit, pending, sourcePath } = useSolarCleaningLeadAction({
    formContext: "solar-cleaning-v0-hero",
    panelCondition,
  });
  ...
  return (
    <>
      ...
      <section className="relative flex flex-col">
        <img
          src="/solaorcleaingImages/IMG_1021.jpeg"
          alt="Technician cleaning solar panels on a tiled roof in Adelaide"
          ...
          {step === "q1" && (
            <div className="w-full max-w-xs rounded-lg bg-card p-4 text-left shadow-2xl md:max-w-md md:p-6">
              ...
              <p className="mb-3 text-center text-sm font-bold leading-snug text-card-foreground">
                What&apos;s the condition of your panels?
              </p>
          ...
          {step === "form" && (
            <div className="w-full max-w-xs rounded-lg bg-card p-4 text-left shadow-2xl md:max-w-md md:p-6">
              ...
              <form action={formAction} onSubmit={handleSubmit} className="space-y-3">
                ...
                <input type="hidden" name="roofCondition" value={panelCondition} />
                <input type="hidden" name="whatTypeOfService" value="solar cleaning" />
                <input type="hidden" name="message" value={`Solar panel condition: ${panelCondition}`} />
                <input type="hidden" name="formSource" value="solar-cleaning-hero" />
                <input type="hidden" name="sourcePath" value={sourcePath} />
                <input type="hidden" name="roofConcern" value={panelCondition} />
                ...
                {FIELDS.map((field) => (
                  <div key={field.key}>
                    <label
                      htmlFor={`solar-hero-${field.key}`}
                      className="mb-1 block text-xs font-semibold uppercase tracking-wide text-muted-foreground"
                    >
                      {field.label}
                    </label>
                    <input
                      id={`solar-hero-${field.key}`}
                      name={field.key}
                      type={field.type}
                      required
