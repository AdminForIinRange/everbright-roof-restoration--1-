...
export function AssessmentForm() {
  ...
  const { error, formAction, handleSubmit, pending, sourcePath } = useV0LeadAction({
    formContext: "roof-restoration-v0-assessment",
    roofCondition: "Free roof assessment request",
      ...
      <div className="mx-auto max-w-lg text-center md:max-w-3xl md:text-left">
        ...
        {submitted ? (
          <div className="rounded-lg border border-border bg-card p-8 text-center shadow-sm">
            ...
            <p className="text-sm text-muted-foreground">Thanks for reaching out. We&apos;ll contact you within 24 hours.</p>
          </div>
        ) : (
          <form
            action={formAction}
          ...
          >
            ...
            <input type="hidden" name="roofCondition" value="Free roof assessment request" />
            <input type="hidden" name="whatTypeOfService" value="roof restoration" />
            <input type="hidden" name="message" value="Free roof assessment request" />
            <input type="hidden" name="formSource" value="roof-restoration-assessment" />
            <input type="hidden" name="sourcePath" value={sourcePath} />
            <input type="hidden" name="roofConcern" value="Free roof assessment request" />
            ...
            <button
              type="submit"
              disabled={pending}
              className="mt-2 w-full rounded-md bg-brand py-3 text-sm font-bold tracking-wide text-brand-foreground transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60 md:col-span-2"
            >
              {pending ? "Submitting..." : <>Start my free roof assessment &rarr;</>}
            </button>
